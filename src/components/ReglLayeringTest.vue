...
// language=GLSL
const VERTEX_SHADER_POSITION_WITH_UV = `
...
onMounted(async () => {
  ...
  const drawShadow = makeShadowRenderer(regl);
  const drawFoil = makeFoilRenderer(regl);
  const drawCorners = makeCornerRenderer(regl);

  const stage1 = regl.framebuffer(rsize.value.width, rsize.value.height);
 ...
 */
function makeShadowRenderer(regl) {
  return regl({
    count: 3,
    attributes: {
      position: [[1, 1], [-3, 1], [1, -3]],
