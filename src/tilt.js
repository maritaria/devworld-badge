...
export function setupTiltByMouse(element) {
  ...
  function onMouseMove(event) {
    ...
    const percent = {
      x: clamp((100 / rect.width) * absolute.x, 0, 100),
      y: clamp((100 / rect.height) * absolute.y, 0, 100),
    };
...
export function applyTilt(element, percent) {
  if (percent) {
    ...
    element.style.setProperty('--pointer-from-center', `${clamp(centerDistance/50, 0, 1).toFixed(2)}`)
    element.style.setProperty('--pointer-from-top', `${clamp(percent.y / 100, 0, 1).toFixed(2)}`);
    element.style.setProperty('--pointer-from-left', `${clamp(percent.x / 100, 0, 1).toFixed(2)}`);
    ...
    element.style.setProperty('--background-x', `${remap(percent.x, 0, 100, 37, 63).toFixed(1)}%`);
    element.style.setProperty('--background-y', `${remap(percent.y, 0, 100, 33, 67).toFixed(1)}%`);
  } else {
